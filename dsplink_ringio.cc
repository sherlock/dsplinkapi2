...
DSPLink_RingIO::DSPLink_RingIO() {
	char dspfilename[]="ringio_new.out";
	dspExecutable = dspfilename;
 ...
 *  ============================================================================
 */
NORMAL_API DSP_STATUS DSPLink_RingIO::RING_IO_Create (IN Uint8 processorId)
{
 ...
 *              */
NORMAL_API DSP_STATUS DSPLink_RingIO::RING_IO_WriteClient1_Initial() {
...
NORMAL_API Uint32 DSPLink_RingIO::RING_IO_WriteClient1_Task(const char *buf_in,
			...
			if (DSP_FAILED(status)) {
				...
				if ((DSP_SUCCEEDED(status)) && (acqSize > 0)) {
					...
					for (i = 0;
							((i < acqSize)
									&& (i + bytesTransfered)
											< RING_IO_BytesToTransfer1); i++) {
						bufPtr[i] = buf_in[(bytesTransfered + i)]
					}
					...
					for (i = 0; i < 5; i++) {
						RING_IO_1Print("    Send [0x%x]  ", bufPtr[i]);
					}
					RING_IO_0Print("\n");
#endif
		...
		while (exitFlag == FALSE) {
			...
			if ((status == RINGIO_SUCCESS) || (acqSize > 0)) {
				...
				for (i = 0; ((i < acqSize) && ((totalRcvbytes + i) < len_out));
						i++)
					buf_out[(totalRcvbytes + i)] = bufPtr[i];
...
NORMAL_API DSP_STATUS DSPLink_RingIO::RING_IO_WriteClient1_Close() {
	DSP_STATUS tmpStatus = DSP_SOK;
	DSP_STATUS status = DSP_SOK;
	do {
...
NORMAL_API DSP_STATUS DSPLink_RingIO::RING_IO_WriteClient2_Initial() {
	DSP_STATUS status = DSP_SOK;
	DSP_STATUS relStatus = DSP_SOK;
	DSP_STATUS tmpStatus = DSP_SOK;
	//RingIO_BufPtr bufPtr = NULL;
...
NORMAL_API Uint32 DSPLink_RingIO::RING_IO_WriteClient2_Task(const char *buf_in,
		const int len_in, char *buf_out, int len_out)
{
	Uint16 type;
	DSP_STATUS status = DSP_SOK;
		...
		while ((RING_IO_BytesToTransfer2 == 0)
			...
			if (DSP_FAILED(status)) {
				...
				if ((DSP_SUCCEEDED(status)) && (acqSize > 0)) {
					...
					for (i = 0; i < 5; i++) {
						RING_IO_1Print("Client2 Send [0x%x]  ", bufPtr[i]);
					}
					RING_IO_0Print("\n");
#endif
...
NORMAL_API DSP_STATUS DSPLink_RingIO::RING_IO_WriteClient2_Close()
{
	DSP_STATUS tmpStatus = DSP_SOK;
	DSP_STATUS status = DSP_SOK;
	do {
...
NORMAL_API Void DSPLink_RingIO::RING_IO_Delete(IN Uint8 processorId)
{
	DSP_STATUS status = DSP_SOK;
	DSP_STATUS tmpStatus = DSP_SOK;
 ...
 */
STATIC NORMAL_API
Void DSPLink_RingIO::RING_IO_Writer_Notify1(IN RingIO_Handle handle,
...
STATIC NORMAL_API
Void DSPLink_RingIO::RING_IO_Writer_Notify2(IN RingIO_Handle handle,
		IN RingIO_NotifyParam param,
		IN RingIO_NotifyMsg msg)
{
	DSP_STATUS status = DSP_SOK;
 ...
 */
STATIC NORMAL_API
Void DSPLink_RingIO::RING_IO_Reader_Notify1 (IN RingIO_Handle handle,
		IN RingIO_NotifyParam param,
		IN RingIO_NotifyMsg msg)
{
	DSP_STATUS status = DSP_SOK;
...
STATIC NORMAL_API
Void DSPLink_RingIO::RING_IO_Reader_Notify2 (IN RingIO_Handle handle,
		IN RingIO_NotifyParam param,
		IN RingIO_NotifyMsg msg)
{
	DSP_STATUS status = DSP_SOK;
